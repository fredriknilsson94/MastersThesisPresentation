...
\begin{frame}<presentation:0>[noframenumbering]{Advanced Driver Assistance Systems (\adas)}
	\begin{columns}
	\column{0.6\textwidth}
	\begin{itemize}
...
\begin{frame}%<presentation:0>[noframenumbering]
	...
	\vspace{1em}
	The homography $\H$ can be decomposed into
	\begin{equation*}
...
\begin{frame}<presentation:0>[noframenumbering]{Filtering Structure}
	\begin{figure}
		\centering
		\begin{tikzpicture}
...
\begin{frame}%<presentation:0>[noframenumbering]
	...
	\begin{itemize}
		\item Trajectories with a moving target were observable
		\item If the target was standing still
		\begin{itemize}
			\item Observable with \roi, angular rate and corner measurements
			\item Unobservable with only \roi and angular rate measurements
		\end{itemize}
		\vspace{1em}
		\item Several state vectors can give the same \roi measurements
	\end{itemize}
	...
	\note
	{
		\begin{itemize}
			...
			\item The target is standing still with an arbitrary initial yaw angle.
			\item The target is driving straight towards or straight away from the host.
			\item The target is driving towards or away from the host applying sine steering.
			\item The target is driving away from the host and makes a $45^\circ$ turn.
			\item The target is driving across the host's lane with constant speed and constant yaw angle.
		\end{itemize}
	}
\end{frame}
